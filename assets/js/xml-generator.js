    ...
    function isValidRequired(rule){
      if (rule.type === "list"){
        // lists[...] exist in global lists below
    ...
    function setTab(which){
      ...
      $("tabIfc").classList.toggle("active", !isManual);
      $("panelManual").style.display = isManual ? "block" : "none";
      $("panelIfc").style.display = isManual ? "none" : "block";
      updateFillProgress();
      markInvalids(isManual ? "manual" : "ifc");
    }
    ...
    $("tabIfc").addEventListener("click", () => {
      if (!ifcAllowed()){
        ...
        // keep manual active, don't switch
    ...
    async function ensureIfcApi(){
      ...
      // IMPORTANT: requires http(s), not file://
    ...
    async function parseIfc(arrayBuffer){
      ...
      for (const t of targets){
        processed++;
        const pct = 8 + (processed/total) * 74; // до ~82%
    ...
    $("btnGenerateFromIfc").addEventListener("click", () => {
      ...
      // overwrite required from IFC preview
    ...
    // If file:// show warning and disable IFC tab by default
    if (!ifcAllowed()){
      $("ifcProtocolWarn").style.display = "block";
      $("tabIfc").classList.add("disabled");
    }
